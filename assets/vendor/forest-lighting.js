...
// A Forest-only art treatment. No simulation state, random stream or source
...
export async function preloadForestAssets() {
  const loader = new T.TextureLoader();
  await Promise.all(files.map(async name => {
    try {
      const texture = await loader.loadAsync(new URL(`../forest/${name}.png`, import.meta.url).href);
      ...
      if (name === 'ground-ao' || name === 'ground-bounce') texture.channel = 1;
      forestAssets[name] = texture;
    } catch (e) { console.warn(`Forest ${name} unavailable; retaining fallback surface.`, e.message); }
  }));
}
...
const outputShader = `varying vec2 vUv; uniform sampler2D source; uniform sampler2D glow;
void main(){vec3 c=texture2D(source,vUv).rgb+texture2D(glow,vUv).rgb*.12;
...
class ForestPost {
  constructor(renderer) {
    ...
    this.output=material(outputShader,{source:{value:null},glow:{value:null}},true);
...
export class ForestLighting {
  constructor({renderer,scene,key,fill,hemi,mat,mode='auto',mobile=false}) {
    ...
    this.active=false;this.post=null;this.envTarget=null;this.slow=0;
    this.mobile=mobile;
  ...
  prepareEnvironment() {
    if(this.envTarget || !this.renderer.isWebGLRenderer || !this.renderer.extensions.has('EXT_color_buffer_float'))return;
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++) {
      ...
      const i=(y*w+x)*4;
      data[i]=.12+sky*.34+sun*3.4;
      data[i+1]=.18+sky*.43+sun*2.7;
      data[i+2]=.10+sky*.62+sun*1.6;data[i+3]=1;
    }
    ...
    try {this.envTarget=pmrem.fromEquirectangular(texture);} finally {texture.dispose();pmrem.dispose();}
  }
  apply(level) {
    this.active=level.id==='forest';
    const on=this.enabled;
    this.key.color.setHex(on?0xffdfad:0xfff2d8);this.key.intensity=on?2.7:2;
    this.key.position.set(on?-6:7,on?13:16,on?5:8);
    this.fill.color.setHex(on?0x9abccc:0x88aaff);this.fill.intensity=on?.48:.5;
    this.fill.position.set(on?8:-9,on?7:8,on?-8:-7);
    this.hemi.color.setHex(on?0xc2dceb:0xbfd8ff);this.hemi.groundColor.setHex(on?0x535132:0x2b3b1e);
    this.hemi.intensity=on?.65:1.05;
    this.renderer.toneMappingExposure=on?1.05:1.15;
    this.scene.background.setHex(on?0x13271e:level.sky);
    this.scene.fog.color.copy(this.scene.background);this.scene.fog.near=on?31:26;this.scene.fog.far=on?54:44;
    if(on) {try{this.prepareEnvironment();}catch(e){console.warn('Forest reflections unavailable',e.message);}}
    this.scene.environment=on?(this.envTarget?.texture||null):null;
    this.scene.environmentIntensity=on?.42:1;
    const d=on?10:12;
    ...
    if(on) {
      const m=this.mat;
      if(forestAssets.ground){if(m.ground.map!==forestAssets.ground)m.ground.map?.dispose();m.ground.map=forestAssets.ground;}
      if(forestAssets.wood){if(m.soft.map!==forestAssets.wood)m.soft.map?.dispose();m.soft.map=forestAssets.wood;}
      m.ground.normalMap=forestAssets['ground-normal']||null;m.ground.normalScale.set(.24,.24);
      m.ground.aoMap=forestAssets['ground-ao']||null;m.ground.aoMapIntensity=.85;
      m.ground.lightMap=forestAssets['ground-bounce']||null;m.ground.lightMapIntensity=1;
      m.ground.roughness=.92;m.soft.roughness=.72;
      for(const name of ['hard','hardTop','wall','wallTop']){m[name].map=forestAssets.stone||null;m[name].roughness=.85;m[name].metalness=0;}
  ...
  render(scene,camera){
    if(this.enabled&&this.tier==='high'&&this.renderer.isWebGLRenderer&&this.renderer.extensions.has('EXT_color_buffer_float')){
      if(!this.post)this.post=new ForestPost(this.renderer);
      this.post.render(scene,camera);
    }else this.renderer.render(scene,camera);
  }
  dispose(){this.post?.dispose();this.envTarget?.dispose();this.post=null;this.envTarget=null;}
}
